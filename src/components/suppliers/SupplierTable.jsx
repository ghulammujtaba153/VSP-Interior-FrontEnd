...
const SupplierTable = () => {
  ...
  const handleImportCSV = () => {
    showConfirmation({
      ...
      message:
        'Are you sure you want to import suppliers from CSV? This will add new supplier records to your database.',
  ...
  const handleDelete = supplierRow => {
    showConfirmation({
      ...
      message: `Are you sure you want to delete supplier "${supplierRow.name}"? This action cannot be undone and will remove all associated data.`,
  ...
  const fetchAllSuppliers = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/suppliers/get?page=1&limit=${totalCount}`) // Large limit to get all
    ...
    return (
      <Box sx={{ margin: 1 }}>
        <Box
          ...
          alignItems='center'
          sx={{ marginLeft: 1, marginRight: 1, mb: 1 }}
        >
          <Typography variant='h6'>Contacts ({contacts.length})</Typography>
            ...
            <TableBody>
              {contacts.map((contact, index) => (
                ...
                >
                  ...
                  <TableCell>
                    <Chip label={capitalizeName(contact.role)} size='small' color='primary' variant='outlined' />
                  </TableCell>
  ...
  return (
    <Paper sx={{ ...layout.container }} className='zoom-67'>
      ...
      <Box sx={{ width: '100%' }}>
              ...
              InputProps={{
                ...
                endAdornment: searchTerm && (
                  <InputAdornment position='end'>
                    <IconButton aria-label='clear search' onClick={handleSearchClear} edge='end' size='small'>
                      <ClearIcon />
                    </IconButton>
                  </InputAdornment>
                )
        ...
        <ViewSupplier open={openView} onClose={() => setOpenView(false)} selectedSupplier={selectedSupplier} />
        ...
        <ImportCSV open={importCSV} onClose={() => setImportCSV(false)} fetchSuppliers={fetchSuppliers} />
                ...
                <TableBody>
                  {suppliers.map((supplier, index) => (
                    <React.Fragment key={supplier.id}>
                      ...
                      >
                        <TableCell>
                          <IconButton size='small' onClick={() => toggleRowExpansion(supplier.id)} color='primary'>
