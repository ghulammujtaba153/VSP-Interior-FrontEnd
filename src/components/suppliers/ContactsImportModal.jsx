...
import { modal, table, button, form, colors } from '@/styles/globalStyles';

const modalStyle = modal;

const ContactsImportModal = ({ open, onClose, supplierId, refreshContacts }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/api/supplier-contacts/import`, { 
    ...
    <Modal open={open} onClose={handleClose}>
      <Box sx={modalStyle}>
        <Typography variant="h5" mb={3} display="flex" alignItems="center" gap={1}>
          ...
          {supplierId && <Chip label={`Supplier ID: ${supplierId}`} color="primary" variant="outlined" />}
        </Typography>
                    ...
                    <TableRow 
                      ...
                      sx={{ 
                        backgroundColor: hasFieldError(index, 'firstName') || 
                          ...
                          ? colors.background.errorRow : 'inherit'
                      }}
                    >
